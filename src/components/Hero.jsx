...
const Hero = () => {
  ...
  const commonTextVariant = textVariant(0);
  ...
  return (
    <section className="relative w-full h-screen mx-auto overflow-hidden">
      <div className={`${styles.paddingX} flex absolute z-10 inset-0 top-[120px] items-start gap-5`}>
        ...
        <div>
          <motion.h1 variants={commonTextVariant} className={`${styles.heroHeadText} funky-text-outline-2`}>
            {text}
          </motion.h1>
          <motion.div variants={fadeIn('', '', 0.1, 1)}>
            <p className={`${styles.heroSubText} funky-text-outline mt-2 text-black-100`}>
              I'm a computer science student with a simple and curious mind.
            </p>
            <p className={`${styles.sectionSubText} mt-2 text-gray-600`}>
              This is just a website showcasing some of my works and experiments. Have a look until you get bored 🥱
            </p>
          </motion.div>
        </div>
      </div>
